...
이 가이드의 각 섹션은 에디터 화면에서 **섹션별로 분리 노출**되도록 설계 (§7 UI 연동 참고).
...
| 공문서·법령·규정·매뉴얼 원문 | §2·§3 템플릿 활용 |
| 레시피·체크리스트·업무 일지 | §3 템플릿 활용 |
| 회의록·학습 정리 | §3 템플릿 활용 |
...
UI 경고 슬롯 → "이 제목을 heading 으로 올릴까요?" 제안 트리거
...
✅  # Ver 1-0         → 단일 분류 "Ver 1-0"
✅  ## Ver 1.0        → 여러 줄 heading 이면 점 있어도 한 단계로 취급
```
...
이 가이드의 각 섹션은 `/note` 라우트의 단일 입력 영역에 보조 UI 로 대응됩니다. 신규 와이어프레임은 [`DESIGN_UI.md`](DESIGN_UI.md) 가 단일 출처.
...
- `DESIGN_INPUT_MODES_AND_RETRIEVAL.md` — 세션 그릇 3 종 + 입력 모드 단일 + 자동저장/의미 처리 분리
- `DESIGN_HYPERGRAPH.md` — 스키마 (`categories` 마스터 + `sentence_categories` + `node_category_mentions` 단일 매핑)
- `DESIGN_CATEGORY.md` — 19 대분류 + 사용자 heading 계층
- `DESIGN_PIPELINE.md` — 자동저장 vs 의미 처리 두 층 분리 + 인출 세 경로

**UI·사용자 경험**
- `DESIGN_UI.md` — 입력 UI 전체 설계

**구현 참조**
- Python frozen: `engine/markdown.py` (마크다운 파서) · `engine/save.py` (저장 파이프라인)
- Flutter: `synapse_engine` 의 `lib/src/markdown/parser.dart` · `lib/src/flow/synapse_flow.dart`

**진행 중 PLAN**
- [`PLAN-20260425-SYN-flutter-rewrite.md`](../deliverables/SYN/20260425/user/PLAN-20260425-SYN-flutter-rewrite.md)
