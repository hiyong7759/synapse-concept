...
**번들 구성**: 베이스 모델 (Gemma 4 E2B-it Q4_K_M GGUF, ~3.1GB) + `enable_thinking=False` + `docs/*_SYSTEMPROMPT.md` 조합이 다수 태스크를 처리. 어댑터 1 종 (`retrieve-expand`, ~52MB) 만 번들 — `save-pronoun`/`meta-filter`/`typo-normalize`/`synapse-answer`/`retrieve-filter` 는 베이스 + 시스템 프롬프트로 처리. 단일 모델 + 앱 번들 고정 정책. 다운로드 인프라·모델 카탈로그·다른 베이스용 어댑터는 후속 PLAN.
