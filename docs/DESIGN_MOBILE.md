...
**번들 구성**: 베이스 모델(Gemma 4 E2B-it GGUF) + `enable_thinking=False` + `docs/*_SYSTEMPROMPT.md` 조합이 다수 태스크를 처리하므로, 모바일에 번들할 어댑터는 3종(`extract-state`, `retrieve-expand`, `retrieve-expand-org`) + 시스템 프롬프트 파일들. 현행 태스크 분담은 `DESIGN_PIPELINE.md` §"태스크 처리 방식" 참고. llamadart 핫스왑 구조는 남은 3개 어댑터에 동일 적용.
