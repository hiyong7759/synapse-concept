...
1. **언급된 것만 존재한다.** 시드 데이터·기본 사전·시스템 노드 없음. 시냅스 = 사용자의 언급 공간과 정확히 일치.
2. **노드는 원자다.** 인물·장소·수치·상태·행위 전부 노드. 품사 무관. 하나의 개념 = 하나의 노드. 중복 생성 금지.
3. **엣지는 문장을 뛰어넘는 의미 관계다.** 조사 기반 엣지 폐기 — 문장 내 문법 구조는 원문 `sentences`로 이미 보존된다. 엣지는 `similar / cooccur / cause / contain` 같은 의미 관계만 담으며, **사후 생성 + 사용자 승인**으로만 `edges`에 들어간다.
4. **저장과 합성의 분리.** 자동 저장 범위는 원문 sentence + 노드 + 노드↔문장 역참조(`node_mentions`) + 치환 실패 토큰(`unresolved_tokens`)까지. 카테고리·엣지·별칭·병합 등 의미 합성은 **모두 사용자 승인**(`/review`)을 거쳐야 DB에 반영된다.
5. **DB에 있는 것은 전부 승인된 것.** `edges`, `node_categories`, `aliases`는 승인된 결과만 담는다. 승인 대기 중인 제안은 저장하지 않고 `/review` 호출 시 런타임에 도출한다.
6. **엣지는 유방향이다.** `source_node_id → target_node_id`. BFS 탐색은 양방향 조회, 저장은 방향 있음.
7. **도메인은 관찰하는 것이다.** 연결이 많아진 노드가 허브가 되고, 허브가 도메인으로 기능한다. 사전 정의하지 않는다.
8. **개인 모드: 1인칭은 문장에 있을 때만 "나" 노드.** 문장에 명시된 1인칭(나/내/저/제)만 "나" 노드로 저장. 문장에 없는 1인칭을 추가하는 것은 금지.
9. **조직 모드: 주어 항상 명시.** 누가 말했는지가 핵심. 1인칭은 사용자 이름으로 치환. 주어 불명확 시 LLM이 질문.
