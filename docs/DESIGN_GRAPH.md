...
**최종 업데이트**: 2026-04-19 (v14 — 자동 저장 + origin 출처 추적, 의미 엣지 자동 생성)
...
node_categories:                     -- 자동 저장 (다대다). origin으로 출처 구분
  node_id     INTEGER REFERENCES nodes(id) ON DELETE CASCADE
  category    TEXT NOT NULL          -- 대분류.소분류 또는 사용자 지정 경로
  origin      TEXT NOT NULL          -- user | ai | rule
  created_at  TEXT DEFAULT (datetime('now'))
  PRIMARY KEY (node_id, category)

edges:                               -- 의미 엣지. 자동 저장 + origin 출처 추적
  id              INTEGER PRIMARY KEY
...
- **자동 저장 + origin (v14)**: `edges / node_categories / aliases`는 전부 자동 저장되며, `origin` 컬럼(`user` / `ai` / `rule`)으로 출처를 식별한다. 사용자는 UI·`/review`에서 origin 필터로 검수·수정·삭제할 수 있다. 파괴적 작업(노드 병합 등)만 `/review` 승인을 거친다.
...
- **edges.label**: 의미 관계만 (`similar / cooccur / cause / contain / …`). 조사 저장 금지.
...
- `domain`, `source`, `weight`, `safety`, `filters`, `nodes.category`, `edges.status` 컬럼 없음.

### origin 값 의미

| 값 | 생성 경로 | 예시 |
|----|----------|------|
| `user` | 사용자가 직접 명시 | 마크다운 heading 경로, 수동 별칭 등록, UI에서 직접 엣지 연결 |
| `ai` | LLM 추론 | extract 어댑터의 카테고리 분류, 의미 엣지 label 자동 생성, 별칭 추천 수락 |
| `rule` | 결정론적 규칙 | 날짜 정규화(`2026-04-18` → `2026년`+`4월`+`18일`), 부정부사(`안/못`) 감지, 형태 매칭 |

UI·`/review`에서는 `origin='ai'` / `'rule'` 필터를 제공해 "AI가 단 것만 검수", "규칙 결과만 검수(규칙 오류 추적)"이 가능하다.
...
조사 엣지가 없어도 BFS 연결성 유지. 의미 관계 엣지는 저장 시 자동 생성되며 `origin='ai'`로 표시. 사용자는 UI에서 즉시 제거·수정 가능.

---

## 엣지 (의미 관계)

자동 생성되는 의미 관계 테이블. `origin`으로 출처 추적.
...
```
저장 파이프라인
  → 같은 sentence의 노드 쌍을 LLM이 관계 label로 판단
  → edges INSERT (origin='ai')

사용자가 UI에서 직접 연결:
  → edges INSERT (origin='user')

규칙 기반 (예: 마크다운 heading contain 관계):
  → edges INSERT (origin='rule')
```

잘못된 엣지는 사용자가 UI에서 "이 연결 제거"로 즉시 삭제. `origin='ai'` 필터 뷰로 한꺼번에 검수 가능.
...
`대분류.소분류` 또는 사용자 지정 경로(마크다운 heading, 예: `더나은.개발팀`) 형식. 다대다 관리, 노드당 복수 카테고리 허용. **모두 자동 저장**, `origin`으로 출처 구분.
...
**용도**: BFS 검색 범위 필터, 뷰 태깅. 잘못된 분류는 `/review`의 AI·규칙 목록 뷰에서 즉시 제거.
...
노드의 다른 이름/변형. 인출 정확도의 핵심. **자동 저장 + `origin` 추적**.

| origin | 생성 경로 | 예시 |
|--------|-----------|------|
| `user` | 직접 등록 ("스벅은 스타벅스야") | `스벅` → 스타벅스 |
| `ai` | 새 노드 생성 시 LLM이 줄임말·외래어·다국어 추천 | `React Native` 노드에 `리액트 네이티브` 자동 등록 |
| `rule` | 형태 매칭 (예: 자모 거리 기반 오타 후보 중 사용자 확정) | `스타벅스` ← `스타벅시` (병합 시 별칭으로 보존) |

잘못된 별칭은 사용자가 노드 상세 화면에서 즉시 제거. `/review`의 `origin='ai'` 목록 뷰로 일괄 검수 가능.
...
doc_mode=True이면 조항 식별자(`제N조`, `제N항`, `N호`)를 노드로 두고, 같은 sentence 내 node_mentions로 본문 노드와 연결. 계층은 `origin='rule'`로 의미 엣지(`contain`)에 자동 반영(규칙 기반).
...
초기: 허리, 감기, 두통, 병원, 약 노드가 각각 존재
      ↓ 연결(node_mentions 공출현 + 저장된 edges, origin 무관) 축적
...
모든 테이블 자동 저장. `edges / node_categories / aliases`는 `origin` 컬럼으로 출처(user/ai/rule)가 식별됨. `/review`는 (1) `unresolved_tokens`(애매한 지시어) 해소 + (2) AI·규칙 생성물 목록 검수 + (3) 파괴적 작업(노드 병합) 승인 세 가지만 담당.
