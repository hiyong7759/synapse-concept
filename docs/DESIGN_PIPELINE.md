...
**v21 변경점**: `node_mentions` → `node_sentence_mentions` 리네임, `node_categories` → `node_category_mentions` 통합 (이전 두 축 흡수). `nodes.status` 폐기 (merge_nodes 물리 DELETE).
...
**핵심 원칙:** `sentence_categories` / `node_category_mentions` / `aliases`는 **자동 저장**되며 `origin` 컬럼으로 출처가 식별된다(`categories` 마스터는 origin 없음 — 삭제 경로 부재). 유일한 승인 대기 예외는 `unresolved_tokens` — 저장 시점에만 감지되는 일회성 이벤트라 재생성 불가. **엣지 테이블(`edges`) 폐기(v15)** — 노드 간 연결은 sentence 공출현·category 공유·alias 묶음이라는 세 종류의 하이퍼엣지로만 표현.
...
모든 하이퍼그래프 변경은 **자동 저장**된다. `sentence_categories`(사용자 heading 계층 문장 매핑) · `node_category_mentions`(노드 ↔ 카테고리 단일 매핑, v21 통합) · `aliases` 모두 저장 시점에 `origin`을 부여받아 즉시 DB에 들어간다. 사용자는 `/review`의 AI·규칙 목록 뷰에서 잘못된 항목을 삭제할 수 있다. 유일한 예외는 `unresolved_tokens`(치환 실패 지시어) + 파괴적 작업(`merge_nodes`, 아카이브)이다.
...
| **의미 처리** | 사용자 명시 트리거 (⌘S / "정리" 버튼 / 재진입 시 제안) | `sentences` 재계산 + Kiwi 노드 + `node_sentence_mentions` + LLM 정정 후보 | ✅ | 0.5~3초 (로컬 LLM — 모바일·데스크톱은 인프로세스 llamadart, Python frozen 환경은 MLX HTTP) | 사용자 자발적 호출만 |
...
사용자 명시 트리거 (⌘S / "정리" 버튼)
  ...
  [메타 필터 — 게시물 단위 1회]                                     ← NEW (v17)
    ...
    로컬 LLM 사용 불가 시 (Python frozen 의 MLX 서버 다운 / 모바일 llamadart 로딩 실패 등) → 필터 skip (모든 문장 저장 진행, 과포함은 UI 삭제로 해소)
  ↓
문장별로 (메타 idx 제외, kind 별 분기):
  ...
  ⑦ DB 저장 (동기 — 즉시 반영):
      ...
      [사용자 heading 계층] (v20 신설, v21 통합 후에도 유지)
      ...
      [노드 ↔ 카테고리 단일 매핑] (v21 통합 — node_categories → node_category_mentions)
...
- **노드 아카이브** (v21 이후 물리 DELETE — `nodes.status` 폐기) — 사용자가 모르게 삭제되면 복구 불가이므로 `stale_nodes` 승인 유지.
...
- **리소스 체크**: CPU/메모리/로컬 LLM 사용 여부 확인 (Python frozen 의 MLX 서버 / 모바일·데스크톱 의 llamadart 인프로세스). 임계치 초과 시 지연
- **백오프**: 외부 API 실패 시 exponential backoff, 반복 실패는 해당 노드 skip 표시

### 워커 ①: 카테고리 분류 워커 (19 대분류 시드 매핑 — v21 통합 후)
...
| Flutter 모바일·데스크톱 (PLAN F2~F5) | Dart `Isolate` 또는 `compute()` + `Semaphore` 패키지 | 워커별 N=2 |
...
질문
  ...
  ↓ [BFS 루프]  max_layers=5
    현재 노드·카테고리 집합마다 세 경로를 합쳐 다음 레이어 구성 (v21 단일 매핑 통합 후):
...
| `ai_generated` | `node_category_mentions WHERE origin='ai'` (시드 카테고리 매핑) · `sentence_categories WHERE origin='ai'` (사용자 heading 계층, 있으면) | 검수 뷰 (유지/삭제) |
...
| 환경 | 런타임 | 베이스 모델 | 호출 방식 |
|---|---|---|---|
| **모바일·데스크톱** (PLAN F3 이후, 프로덕션) | `llamadart` (llama.cpp 바인딩) — `synapse_engine` 패키지 내부 | Gemma 4 E2B-it 4bit GGUF (~1.2GB, 앱 번들) | **인프로세스 직접 호출** (HTTP 없음, 레이턴시 0) |
...
| typo-normalize (v22 2차안 신설) | 베이스 모델 | `docs/TYPO_NORMALIZE_SYSTEMPROMPT.md` (PLAN F3 신설 예정) — 별칭 보호 + 자모 거리 사전 필터 통과 토큰 검증 |
| retrieve-filter | 베이스 모델 | `docs/RETRIEVE_FILTER_SYSTEMPROMPT.md` |
| retrieve-expand | 어댑터 | `synapse/retrieve-expand` (v22 2차안 단계 모바일·데스크톱 번들 1 종) |
| retrieve-expand-org | 어댑터 | `synapse/retrieve-expand-org` (보류 — 후속 PLAN, v22 2차안 단계 미번들) |
| category (백그라운드) | 베이스 모델 | `docs/CATEGORY_SYSTEMPROMPT.md` |
| synapse-answer (이전 chat) | 베이스 모델 | `docs/SYNAPSE_ANSWER_SYSTEMPROMPT.md` (PLAN F3 신설 예정) |
...
시스템 프롬프트 파일 로더는 환경별로 갈라진다 — Python frozen: `engine/prompts.py` / Flutter (PLAN F3): `synapse_engine` 의 `lib/src/llm/prompts/` assets 로더. 권한 판단(security-access 등)은 결정론적 백엔드 로직 소관이라 LLM 태스크에서 제외. v22 2차안 단계에서는 보안·민감도 LLM 태스크가 모두 후속 PLAN 으로 이관 (`DESIGN_FINETUNE.md` Task 5A/5B/5A-org/5B-org 폐기 섹션 참조).
...
| 정정 후보 (typo-normalize, v22 2차안 신설) | 베이스 + TYPO_NORMALIZE_SYSTEMPROMPT.md | 0 | 256 |
...
v22 2차안 단계 폐기·이관:
- `라우팅` (ROUTING_SYSTEMPROMPT.md) — 채팅 단일 흐름이 폐기되어 라우팅 LLM 태스크 사장. `/note`·`/synapse` 라우트는 사용자 명시 결정.
- `민감정보 확인` (security-context) — 보안 LLM 태스크는 후속 PLAN (`DESIGN_FINETUNE.md` 5A/5B 폐기 섹션 참조).
...
## assistant 응답 하이퍼그래프 편입 (후속 PLAN)

v22 2차안 단계: `role='assistant'` sentences 에만 저장. 하이퍼그래프 추출 안 함. 단, **사용자가 명시적으로 [⬆ 통찰로 승격] 클릭 시** 본체 sentence 가 `origin='insight'` 로 편입되며 시냅스 세션 retrieve 캐시 노드 전부와 허브 연결 (원칙 15-2).

자동 편입 미도입 이유: 2B 로컬 모델 응답이 사용자 말 되풀이 수준. 사용자 의도 없는 자동 편입은 노이즈 가능.

**재검토 조건 (후속 PLAN 트리거):**
- 모델 품질 향상 (응답에 새로운 인사이트/분석 포함)
- 도구 확장으로 외부 데이터 수집 시
- 통찰 승격 사용 패턴 분석 후 자동 편입 정책 재설계
