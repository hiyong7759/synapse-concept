...
카테고리는 `node_categories` 테이블에 **자동 저장**되며 `origin` 컬럼으로 출처를 식별한다. 사용자는 `/review`의 `ai_generated`/`system_generated` 목록 뷰에서 잘못된 분류를 즉시 삭제할 수 있다.

| origin | 생성 경로 | 예시 |
|--------|-----------|------|
| `user` | 마크다운 heading 경로 (사용자 명시) | `# 더나은\n## 개발팀` → `더나은.개발팀` |
| `ai` | 백그라운드 워커 — 베이스 모델 + `CATEGORY_SYSTEMPROMPT.md` | `허리디스크` 노드 → `BOD.disease` |
| `system` | 결정론적 엔진 규칙 (v17 — 이전 `rule`) | 날짜 분할(`4월` → `TIM.month`), 부정부사(`안/못` 표식) |
...
- 카테고리 시스템 프롬프트: `docs/CATEGORY_SYSTEMPROMPT.md` (베이스 모델 주입)
- 카테고리 분류 워커: `engine/workers.py` — `category_worker()`
