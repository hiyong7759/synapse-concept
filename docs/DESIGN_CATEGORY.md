...
3. **사용자 heading 계층** — `categories` 재귀 CTE + `sentence_categories` (사용자 heading 서브트리 전체 sentence 수집)

`categories` 마스터는 두 가지를 동시에 담는다:
...
**1. 연결 기준**
같은 카테고리를 공유하는 노드들은 서로 연결된 것으로 간주한다. 예: `스타벅스`와 `투썸` 두 노드가 `FOD.restaurant` 시드 카테고리를 공유하면 (`node_category_mentions` 두 행), 스타벅스가 언급된 질문에도 투썸 관련 sentence 가 후보로 떠오른다.
...
"허리 아파?" 질문 → BOD.* 시드 카테고리 → node_category_mentions 역조회 → 강남세브란스 도달 가능
```

**3. 소형 모델의 인출 범위 한계 보완**
2B 모델은 질문 의도에서 인접 카테고리까지 추론하기 어렵다. 카테고리 인접 맵을 코드에 하드코딩하여 모델 판단 없이 탐색 범위를 자동 확장 (19 대분류 시드 카테고리 간 인접 맵 — 사용자 heading 계층은 `categories.parent_id` 재귀 CTE 로 서브트리 자동 확장).
...
- `categories`: `(id, name, parent_id, created_at)`, `UNIQUE(parent_id, name)`. adjacency list.
  - 19 대분류 + 약 100 개 소분류 시드 루트 (`parent_id=NULL`)
  - 사용자 heading 경로가 추가로 누적
  - origin 없음 (삭제 경로 부재. 시드 보호는 애플리케이션 상수)
- `sentence_categories`: heading 말단 카테고리만 문장에 연결. `origin` 컬럼(`user|ai|system|external`) 유지.
...
- AI 워커는 19 대분류 시드로, heading Kiwi 토큰화·사용자 수동 교정은 임의 카테고리로 매핑.
- 한 노드가 여러 카테고리에 속할 수 있다 (다대다).

| origin (node_category_mentions) | 생성 경로 | 예시 |
|--------|-----------|------|
| `ai` | 백그라운드 워커 — 베이스 모델 + `CATEGORY_SYSTEMPROMPT.md` | `허리디스크` 노드 → `BOD.disease` 시드 카테고리 id |
| `system` | 결정론적 엔진 규칙 (Kiwi heading 토큰화·날짜 분할 등) | `4월` 노드 → `TIM.month` 시드 카테고리 id, heading 노드 자동 매핑 |
| `user` | `/review` 수동 교정 | 잘못 분류된 항목 수정 |

### 계층 확장 규칙

- **루트 네이밍 충돌 방지**: 사용자 heading 루트는 시드된 19 대분류 이름(예: `건강`·`자연`)과 겹칠 수 있다. 이름이 동일하면 시드 루트에 붙여서 하위 노드로 편입하지 않는다 — 사용자 의도는 "건강" 이란 범주 자체가 아니라 내 개인 카테고리일 수 있으므로 분리. heading path 의 루트 세그먼트가 시드 루트 이름과 일치하면 경고 후 사용자 승인 이전엔 별도 루트로 유지.
- **경로 비교는 id 기반**: 문자열 split 대신 `parent_id` 체인 조회. 경로 조립은 재귀 CTE 혹은 애플리케이션 조립.
- **리네이밍 안전**: `categories.name` 변경 시 `sentence_categories` · `node_category_mentions` 참조는 FK 로 그대로 유지.
...
> **19 대분류 시드 카테고리 간 관계 전용** — 인접 맵은 `BOD.disease` 같은 19 대분류 시드 코드 사이에만 정의한다. 사용자 heading 계층은 `categories.parent_id` 기반 재귀 CTE 로 이미 서브트리 확장을 지원하므로 인접 맵이 필요 없다.
...
단방향으로 정의, 코드에서 역방향 자동 포함. Python frozen 환경 (`engine/retrieve.py`) 과 Flutter 환경 (`synapse_engine` `lib/src/graph/bfs.dart`) 양쪽에서 같은 페어 목록을 사용.
...
FROM node_sentence_mentions m1
JOIN node_sentence_mentions m2 ON m1.sentence_id = m2.sentence_id
                              AND m1.node_id < m2.node_id
JOIN node_category_mentions c1 ON c1.node_id = m1.node_id
JOIN node_category_mentions c2 ON c2.node_id = m2.node_id
JOIN categories cat_a ON cat_a.id = c1.category_id
JOIN categories cat_b ON cat_b.id = c2.category_id
WHERE c1.category_id != c2.category_id
  AND cat_a.parent_id IS NULL  -- 시드 루트만
  AND cat_b.parent_id IS NULL
