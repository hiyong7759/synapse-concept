...
- **소분류 레벨** — 대분류 레벨 연결은 범위가 너무 넓어 노이즈 증가. 소분류로 좁혀 정밀도 확보.
- **크로스 대분류만** — 같은 대분류 내 소분류 간 연결은 기존 BFS 보완(대분류 전체 조회)으로 이미 처리됨.
- **단방향 정의, 양방향 적용** — 코드에서 역방향 자동 포함.
- **약한 연관은 제외** — "질문 시 같이 보면 답이 확실히 나아지는가"가 기준.
- **데이터 기반 검증** — 데이터가 쌓이면 카테고리 간 엣지 빈도로 보완·조정.
