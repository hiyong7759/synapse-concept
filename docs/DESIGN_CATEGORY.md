...
**최종 업데이트**: 2026-04-23 (v20 계획 — PLAN-004 "카테고리 재설계" · 사용자 heading 계층(`categories`·`sentence_categories`) 과 19 대분류 의미 태깅(`node_categories`) 을 **두 축**으로 분리. v15 분류체계·인접 맵은 그대로 유효)

## 목적

v20 에서 카테고리는 **두 축**으로 나뉜다. 엣지 테이블 없는 구조에서 노드 간 "의미적 연결"은 ① `node_mentions` 공출현, ② 축 B `node_categories.major_category` 공유(+ 인접 맵 한 홉), ③ 축 A `sentence_categories` + `categories` 서브트리 — 세 경로로 드러난다.

**축 A — 사용자 heading 계층** (`categories` + `sentence_categories`, v20 신설)
사용자가 마크다운 heading 으로 명시한 경로를 adjacency list(`parent_id` FK) 로 정규화. 문장 단위 주 매핑.
- 예: `# 더나은\n## 개발팀` → `categories` `(더나은, NULL)`·`(개발팀, 더나은.id)`, 해당 heading 하위 문장은 `sentence_categories(sentence_id, 개발팀.id, 'user')`
- 질의 "더나은 전체 보여줘" → 재귀 CTE 로 하위 id 수집 → `sentence_categories` JOIN 1 번으로 모든 하위 문장 반환
- 문자열 split 의존 제거, 중간 리네이밍·구조 변경 안전

**축 B — 19 대분류 의미 태깅** (`node_categories`, v20 재정의)
노드 의미를 19 대분류 코드(`CATEGORY_SYSTEMPROMPT.md`, 약 100 개 고정)로 태깅. 저장은 노드 단위.
- 예: `민지 → PER.individual`, `허리디스크 → BOD.disease`
- CATEGORY 백그라운드 워커 전용 (규칙 기반 예외: 날짜 노드 `TIM.*` 자동)
- heading 경로 문자열은 저장 금지 (축 A 로 분리)

### 카테고리가 하는 세 가지 일

**1. 연결 기준**
같은 카테고리를 공유하는 노드들은 서로 연결된 것으로 간주한다. 예: `스타벅스`와 `투썸` 두 노드가 `FOD.restaurant` (`major_category`) 를 공유하면, 스타벅스가 언급된 질문에도 투썸 관련 sentence 가 후보로 떠오른다.

**2. BFS 연결 부재 보완**
파편화된 입력으로 같은 문장 바구니로 연결되지 않는 노드를 카테고리 기준으로 추가 조회.
```
"허리디스크 L4-L5 진단받았어"  → 저장
"강남세브란스 정형외과 다니고 있어"  → 별도 문장으로 저장 → 문장 바구니 미연결
"허리 아파?" 질문 → BOD.* 의미 태깅(축 B) 보완 → 강남세브란스 도달 가능
```

**3. 소형 모델의 인출 범위 한계 보완**
2B 모델은 질문 의도에서 인접 카테고리까지 추론하기 어렵다. 카테고리 인접 맵을 코드에 하드코딩하여 모델 판단 없이 탐색 범위를 자동 확장 (축 B 전용 — 축 A 는 재귀 CTE 로 서브트리 자동 확장).

---

## 저장 방식 (v20)

### 축 A — `categories` + `sentence_categories`

- `categories` 마스터: `(id, name, parent_id, created_at)`, `UNIQUE(parent_id, name)`. adjacency list. 저장 경로에서 heading path segments 를 상위→하위로 upsert.
- `sentence_categories` 연결: heading **말단** 카테고리만 문장에 연결. `origin` 컬럼(`user|ai|system|external`) 유지.
- 19 대분류 시드: 마이그레이션 시점에 `CATEGORY_SYSTEMPROMPT.md` 파싱해 `categories` 에 시드 루트 19 개 + 소분류 약 100 개 INSERT (루트의 `parent_id=NULL`).
- origin 없음 (`categories` 마스터): 삭제 경로가 UI 에 없어 시드 보호는 애플리케이션 상수로 처리.

| origin (sentence_categories) | 생성 경로 | 예시 |
|--------|-----------|------|
...
### 축 B — `node_categories` (재정의)

- 스키마: `(node_id, major_category TEXT, origin, created_at)`, PK `(node_id, major_category)`.
- `major_category` 는 `CATEGORY_SYSTEMPROMPT.md` 코드 상수 — 예: `BOD.disease`, `TEC.hw`.
- FK 불필요 — 값은 고정 상수, 애플리케이션에서 validation.

| origin (node_categories) | 생성 경로 | 예시 |
|--------|-----------|------|
| `ai` | 백그라운드 워커 — 베이스 모델 + `CATEGORY_SYSTEMPROMPT.md` | `허리디스크` 노드 → `BOD.disease` |
| `system` | 결정론적 엔진 규칙 | 날짜 분할(`4월` → `TIM.month`) |
| `user` | `/review` 수동 교정 | 잘못 분류된 항목 수정 |

한 노드가 여러 `major_category` 를 가질 수 있다(다대다).

### 계층 확장 규칙 (축 A)

- **루트 네이밍 충돌 방지**: 사용자 heading 루트는 시드된 19 대분류 이름(예: `건강`·`자연`)과 겹칠 수 있다. 이름이 동일하면 시드 루트에 붙여서 하위 노드로 편입하지 않는다 — 사용자 의도는 "건강" 이란 범주 자체가 아니라 내 개인 카테고리일 수 있으므로 분리. 마이그레이션·저장 로직에서 heading path 의 루트 세그먼트가 시드 루트 이름과 일치하면 경고 후 사용자 승인 이전엔 별도 루트로 유지 (M3 에서 세부화).
- **경로 비교는 id 기반**: 문자열 split 대신 `parent_id` 체인 조회. 경로 조립은 재귀 CTE 혹은 애플리케이션 조립.
- **리네이밍 안전**: `categories.name` 변경 시 `sentence_categories` 참조는 FK 로 그대로 유지.
...
> **축 B 전용** — 인접 맵은 19 대분류 코드(`major_category`) 간 관계만 정의한다. 축 A(사용자 heading 계층)는 `categories.parent_id` 기반 재귀 CTE 로 이미 서브트리 확장을 지원하므로 인접 맵이 필요 없다.
...
-- v20: node_categories.major_category (축 B) 기준
SELECT
    c1.major_category AS cat_a,
    c2.major_category AS cat_b,
...
WHERE c1.major_category != c2.major_category
...
- 스키마: `docs/DESIGN_HYPERGRAPH.md` — 축 A(`categories` + `sentence_categories`) · 축 B(`node_categories.major_category`)
- 인출 파이프라인: `docs/DESIGN_PIPELINE.md` — 세 경로 BFS (문장 바구니 + 축 B + 축 A)
- 카테고리 시스템 프롬프트: `docs/CATEGORY_SYSTEMPROMPT.md` (베이스 모델 주입 — 축 B 워커 전용)
- 카테고리 분류 워커: `engine/workers.py` — `category_worker()` (축 B 전용)
- 구현: `engine/retrieve.py` — `_get_category_supplement_nodes()` (축 B) · 축 A 재귀 CTE 헬퍼 (v20 PLAN-004 M2 에서 추가)
- PLAN: `deliverables/SYN/20260422/user/PLAN-20260422-SYN-004-category-redesign.md`
