...
## 2026-04-20 현행 상태 — 어댑터 ↔ 베이스 모델 전환 현황

Gemma 4 E2B-it 의 thinking 모드를 OFF로 고정(`enable_thinking=False`)한 뒤, 학습 데이터 system 메시지 그대로를 시스템 프롬프트로 주입하는 방식으로 여러 태스크가 **베이스 모델만으로 파인튜닝을 능가**함을 실측. 아래 표는 단일 베이스 모델 + 시스템 프롬프트 기준 정답률.
...
- `security-access` · `security-org` · `security-personal`

**thinking OFF 효과**: ON 상태에서는 routing 0% / retrieve-filter 13% / save-pronoun 0% / security-context 66.7% 였으나 OFF 적용 후 위 수치로 개선. 추론 시간도 약 10배 감소. 상세: `docs/DESIGN_PIPELINE.md` §"Gemma 4 thinking 모드 OFF".
...
별칭 제안은 외부 지식베이스(Wikidata)로 전환. 파인튜닝 불필요.

| # | 태스크 | 입력 | 출력 | 상태 |
|---|--------|------|------|------|
| ~~1~~ | ~~상태 변경 감지~~ | — | — | **폐기** — Task 6 deactivate 필드로 통합 |
| ~~2~~ | ~~대명사/부사/지시어 구체화~~ | 입력 텍스트 + 맥락 | 치환된 텍스트 | **베이스 모델 전환** — `SAVE_PRONOUN_SYSTEMPROMPT.md` |
| ~~3~~ | ~~인출 필터~~ | 문장 + 질문 | pass / reject | **베이스 모델 전환** (96.7%) — `RETRIEVE_FILTER_SYSTEMPROMPT.md` |
| 4 | 인출 확장 | 질문 | 관련 노드 후보 목록 | **어댑터 유지** (retrieve-expand / -org) |
| ~~5A~~ | ~~관계 민감도 마킹~~ | 관계 하나 | safe/sensitive | **폐기** — 백엔드 권한 체계 |
| ~~5B~~ | ~~컨텍스트 민감도 판단~~ | 질문 + 관계들 | safe/confirm | **베이스 모델 전환** (security-context, 90%) |
| ~~6A~~ | ~~노드 추출 (extract-core)~~ | 한국어 문장 | {"nodes":[...]} | **베이스 모델 전환** (94.4%) — `EXTRACT_SYSTEMPROMPT.md` |
| **6B** | **상태변경 탐지 (extract-state)** | 현재 발화 + 알려진 사실 | {"deactivate":[sid...]} | **어댑터 유지** |
...
> **⚠️ 2026-04-20 업데이트 — Task 6A extract-core 는 어댑터 폐기.**
> 베이스 모델 + `docs/EXTRACT_SYSTEMPROMPT.md` + `enable_thinking=False` 로 94.4% 달성 (파인튜닝 최고 84.7%).
> 아래 내용은 분리 설계 근거를 남기기 위한 역사적 기록. **현재 extract-state 만 어댑터 유지**.
> 학습 데이터 `data/finetune/tasks/extract-core/` 는 보존 (재학습 근거 자료로).
...
| 구분 | 태스크 | 총 건수 | 상태 (2026-04-20) |
|------|--------|---------|------|
| Personal | ~~Task 2 save-pronoun~~ | 720건 | 베이스 전환 — 데이터 보존 |
| Personal | ~~Task 3 retrieve-filter~~ | 1,400건 | 베이스 전환 (96.7%) — 데이터 보존 |
| Personal | Task 4 retrieve-expand | 468건 | **유지** (어댑터) |
...
| 어댑터 | n_train | iters | 상태 (2026-04-20) |
|---|---|---|---|
| ~~extract-core~~ | ~~1798~~ | ~~1348~~ | **폐기** — 베이스 모델 전환 (94.4%) |
| extract-state | 490 | 367 | **유지** |
| ~~retrieve-filter~~ | ~~1269~~ | ~~951~~ | **폐기** — 베이스 모델 전환 (96.7%) |
...
| ~~security-access~~ | ~~319~~ | ~~239~~ | **폐기** — 백엔드 권한 체계 |
| ~~security-org~~ | — | — | **폐기** — 백엔드 권한 체계 |
| ~~security-personal~~ | ~~495~~ | ~~371~~ | **폐기** — 백엔드 권한 체계 |
