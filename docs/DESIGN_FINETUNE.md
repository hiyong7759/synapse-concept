...
**최종 업데이트**: 2026-04-25 (v22 2차안 — 단일 모델 (Gemma 4 E2B 4bit GGUF) + 어댑터 1 종 (`retrieve-expand`) 정책 명시. `extract`·`extract-merge`·`structure-suggest` (v17 폐기) / `extract-state` + `sentences.status` (v18 폐기) / 트리플 (source—label—target) 표현 (v15 엣지 폐기) 본문 정합. `typo-normalize` 태스크 신설 (v22 2차안 LLM 정정 후보 생성). Flutter 환경 (`synapse_engine` 의 `LlmTasks`) 어댑터 번들·핫스왑 정책 명시. Python frozen 환경의 MLX 학습 파이프라인은 그대로 유지 — 모바일·데스크톱은 학습된 어댑터를 GGUF 변환해 번들.)
...
| typo-normalize (v22 2차안 신설) | 베이스 + `docs/TYPO_NORMALIZE_SYSTEMPROMPT.md` (PLAN F3 신설 예정) | LLM 정정 후보 생성 — 별칭 보호 + 자모 거리 사전 필터 통과 토큰 검증, 사용자 [적용] 클릭 강제 |
| retrieve-filter | 베이스 + `docs/RETRIEVE_FILTER_SYSTEMPROMPT.md` | 인출된 문장의 질문 관련성 |
| retrieve-expand | 파인튜닝 어댑터 | 질문 → BFS 탐색 노드 후보 |
| retrieve-expand-org | 파인튜닝 어댑터 | retrieve-expand 조직 버전 (v22 2차안 단계 후순위 — 후속 PLAN) |
| synapse-answer | 베이스 + `docs/SYNAPSE_ANSWER_SYSTEMPROMPT.md` (이전 chat 어댑터 대체) | 시냅스 한 턴 답변 합성 |
| category (백그라운드 워커) | 베이스 + `docs/CATEGORY_SYSTEMPROMPT.md` | 노드 카테고리 자동 분류 |

v17 폐기: `extract`, `extract-merge`, `structure-suggest` — Kiwi 단독이 저장 기본 경로.
v18 폐기: `extract-state` + `sentences.status` 컬럼 — 저장 시점 해석 폐기. 시점 해석은 인출 LLM 의 `created_at` 기반 최근성 판단으로 이관.
v22 2차안 단계: 어댑터는 `retrieve-expand` 1 종만 모바일·데스크톱 번들 (단일 모델 + 앱 번들 고정 정책). 다른 베이스 모델용 어댑터·도메인 특화 어댑터는 후속 PLAN.

**현행 파인튜닝 어댑터 (학습·번들 활성)**: `retrieve-expand` 1 종.
**보류 어댑터** (학습 데이터 보존, v22 2차안 단계 미번들): `retrieve-expand-org`.
...
### ~~태스크 5A: 관계 민감도 마킹~~ (v15 폐기)

> 엣지 테이블 자체가 v15 에서 폐기되어 "관계 (source—label—target)" 단위 마킹 개념이 소멸. 민감도 판단이 필요하면 sentence 단위 또는 노드 단위로 재정의해 별도 PLAN 에서 다룬다.

**v15 이전 시스템 프롬프트 (참고용 보존):**
```
...
### ~~태스크 5B: 컨텍스트 전체 민감도 판단~~ (v15 폐기)

> 5A 폐기와 함께 사장. 인출 컨텍스트의 민감도 판단은 sentence 단위로 재정의해 별도 PLAN 에서 다룬다.
...
구조 동일 (sentence pass/reject). v15 엣지 폐기 후 doc_mode 도 sentence 단위로 처리 — `35조`·`35조①` 같은 조항 식별자는 노드, 본문 내용은 sentence, 계층은 `categories` + `sentence_categories` 로 표현.

```
질문: "연차가 며칠이야?"
sentence: "제35조 (연차) 1년 동안 80% 이상 출근한 근로자에게 15일의 유급휴가를 준다"   → pass
sentence: "제35조①에 따라 출근율을 산정한다"                                          → pass
sentence: "김팀장은 개발팀 소속이다"                                                   → reject
```
...
### ~~Task 5A-org: 트리플 민감도 마킹~~ (v15 폐기)

> 엣지 테이블 폐기로 트리플 단위 마킹 개념 소멸. 권한 기반 민감도 판단은 sentence·노드 단위로 재정의해 후속 PLAN (조직 모드 v22 적용) 에서 다룬다.

**v15 이전 정의 (참고용 보존):**

트리플 민감도 마킹 (최소 열람 권한 포함)
...
### ~~Task 5B-org: 컨텍스트 민감도 판단~~ (v15 폐기)

> 5A-org 폐기와 함께 사장. 후속 PLAN 에서 sentence·노드 단위로 재정의.
...
## ~~태스크 6: extract + extract-state~~ (v17/v18 폐기)

> v17 에서 `extract`·`extract-merge` 폐기 (Kiwi 단독 저장 경로 채택). v18 에서 `extract-state` + `sentences.status` 폐기 (저장 시점 해석 폐기). 본 섹션은 v15~v17 시점 어댑터 설계의 참고용 보존.

**v15~v17 배경**: Kiwi 형태소 분석 기반 저장 파이프라인의 한계.
...
### ~~Task 6A: extract-core~~ (v17 폐기)

> v17 에서 Kiwi 단독 저장 경로 채택. extract LLM 어댑터 폐기. 본 섹션은 v15~v17 시점 어댑터 설계 참고용 보존.

**v15~v17 정의:** extract-core (노드 추출, v13)
...
### ~~Task 6B: extract-state~~ (v18 폐기)

> v18 에서 `sentences.status` 컬럼 + `extract-state` LLM 판정 전면 폐기. 시점 해석은 인출 LLM 의 `created_at` 기반 최근성 판단으로 이관. 본 섹션은 v15~v17 시점 어댑터 설계 참고용 보존.

**v15~v17 정의:** extract-state (모순 탐지)
...
> v22 2차안 단계: 학습 환경은 그대로 Mac M4 + MLX (Python frozen). 학습된 어댑터를 GGUF 변환 (`scripts/convert_adapters.py` — PLAN F3 진입 전 작성) 후 모바일·데스크톱 앱 번들 (`synapse_engine` 패키지의 `assets/adapters/`) 에 포함. 모바일·데스크톱은 학습하지 않음 (재학습은 항상 MLX 환경에서).
...
3. **남은 3개 어댑터 재학습 필요 여부 판단** — 현재 학습 파이프라인(MLX LoRA)로 재현 가능한지 검증.
