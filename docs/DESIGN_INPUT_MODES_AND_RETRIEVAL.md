...
**최종 업데이트**: 2026-04-25 (v22 반영 — M3 에서 스키마 실 구현 완료. 2 모드 → **4 모드** 로 확장. `posts.input_mode` → `kind`. 인출·융합 모드 `synapse` 1급화. 사용자 승격 통찰 `insight` 1급화. 선행 v19: PLAN-003 "마크다운 입력 체계 + 모드 분리" — `posts.input_mode` 컬럼 신설 / `- key:: value` 파서 / save-pronoun 모드별 분기)
...
사용자 입력 (단일 줄 또는 여러 줄 평문)
  ↓ posts INSERT (kind='chat', source=원문, title=첫 행 자동)
...
사용자 입력 (heading + key-value + list + 자유 문장 혼재)
  ↓ posts INSERT (kind='markdown', source=원문, title=첫 행 자동)
