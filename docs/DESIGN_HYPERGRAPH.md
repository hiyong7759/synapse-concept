...
**최종 업데이트**: 2026-04-25 (v22 2차안 — 입력 모드 분리 폐지로 kind 4값 → **3값** (`note`/`synapse`/`insight`). `chat`·`markdown` 통합 → `note` 단일 축적 그릇. M3 스키마는 실 구현 완료 (M3.1 미세 조정으로 CHECK 값에서 `chat` 제거 예정). 선행 v22 1차안: 4 값(chat/markdown/synapse/insight) 안. `posts.input_mode`→`kind` 리네임, `posts.title` 신설, `posts.markdown`→`source` 리네임, `sentences.post_id NOT NULL` 강제, `sentences.origin` 신설(`insight` 표식). 선행 v21: PLAN-007 "하이퍼엣지 매핑 단일화" — `node_mentions`→`node_sentence_mentions`, `node_categories`→`node_category_mentions` 통합. v20: PLAN-004 카테고리 재설계. v19: `posts.input_mode` 신설. v18: 상태 레이어 제거. v17: Kiwi-first + 메타 필터 + origin `rule→system`)
...
> v21 → v22 변경 (앱 재작성 동반, M3 에서 `engine/db.py` 실 구현 완료, M3.1 에서 v22 2차안 미세 조정 예정): **post 의 의미 확장 = "세션 그릇"**. 축적(노트)·시냅스(인출·융합)·통찰(승격) 3 종 통용.
> - **리네임 `input_mode` → `kind`** — "입력 모드" 뉘앙스가 `synapse`·`insight` 와 어긋남. **v22 2차안 최종**: `kind ∈ {'note','synapse','insight'}`. 입력 모드(chat/markdown) 분리 폐지로 축적 그릇은 `note` 단일.
> - **리네임 `markdown` → `source`** — 그릇 종류마다 의미가 다름 (note 는 사용자가 친 원본, synapse 는 Q/A 로그, insight 는 본체 한 줄). nullable. **세션 전체 원본**을 누적 보관 (자동저장이 1.5초 디바운스로 갱신).
> - **신설 `title`** — 목록 표시용 제목. 기본값은 첫 sentence 의 첫 행 자동 세팅, 사용자 편집 가능. NULL 허용.
> - **신설 `sentences.origin TEXT NULL`** — 통찰 승격 sentence 식별 (`NULL` 또는 `'user'` 또는 `'insight'`). 일반 sentence 는 NULL/`'user'`, 통찰 본체는 `'insight'` (편집 불가).
> - **강제 `sentences.post_id NOT NULL`** — "post 에 속하지 않는 문장" 폐지. assistant 응답도 그 대화가 일어난 post 에 `role='assistant'` 로 귀속. 모든 sentence 는 어떤 세션 그릇에 반드시 속함.
> - **통찰 허브 연결** — `kind='insight'` post 생성 시, 승격 직전 시냅스 세션의 retrieve 결과 노드 전부와 `node_sentence_mentions` 자동 일괄 INSERT. 본문 언급 여부 무관 (Hebbian 구현).
>
> v20 → v21 변경 (PLAN-007 M2·M3): **하이퍼엣지 매핑 단일 대칭 구조**.
> - **리네임 `node_mentions` → `node_sentence_mentions`** — 이름이 모호했던 걸 명시화. `origin` 컬럼 추가 (`'system'` 기본, v17 리네이밍 반영).
> - **신설 `node_category_mentions`** — 노드 ↔ 카테고리 멤버십. `_upsert_category_path` 가 heading 을 Kiwi 로 토큰화해 자동 매핑 (`origin='system'`). AI 워커는 `origin='ai'`, 사용자 수정은 `'user'`.
> - **폐기 `node_categories`** — 19 대분류 TEXT 전용 테이블이었지만 `categories` 트리와 분리된 이중 시스템. `node_category_mentions` 로 흡수 (19 대분류 시드도 `categories` 트리의 노드이므로 `categories.id` FK 로 통합).
> - **폐기 `nodes.status`** — `merge_nodes` 의 soft-delete 마커로만 쓰이던 legacy. 물리 `DELETE FROM nodes` 로 전환. 원칙 10 "도메인은 관찰" 위배 해소.
> - **인출 경로 교체** — `_match_start_categories` 를 categories.name exact match → `node_category_mentions` JOIN + 재귀 CTE 로 교체. Kiwi 가 heading 의 의미 토큰을 노드로 뽑아놨기에 질문 키워드 → 노드 매칭 → 카테고리 서브트리 시드 경로가 자연스럽게 성립.
> - **축 A·B 단일화** — 기존 축 A(사용자 heading) / 축 B(19 대분류) 구분 폐기. 한 매핑 테이블로 통합. `_ADJACENT_PAIRS`(72 쌍 수동 정의) + `_get_category_supplement_nodes` 도 "같은 카테고리 구성원 노드 역조회" 로 데이터 주도 확장.
>
...
> - **재정의 `node_categories`** — (v21 에서 폐기됨) 19 대분류 의미 태깅 전용. `category TEXT` → `major_category TEXT`.
...
```sql
posts:                               -- 세션 그릇 (v22 2차안 — 3 값)
  id          INTEGER PRIMARY KEY
  kind        TEXT NOT NULL DEFAULT 'note'
                  CHECK(kind IN ('note','synapse','insight'))
                                               -- v22 2차안: 입력 모드 분리 폐지로
                                               -- chat·markdown 통합 → note 단일.
                                               -- note: 지식 축적 (뉴런 재료, /note 화면)
                                               -- synapse: 인출·융합 세션 (발화 순간)
                                               -- insight: 승격된 통찰 고유 그릇 (허브)
  title       TEXT                              -- v22 신설. 목록 표시용 제목.
                                               -- 기본값: 첫 sentence 의 첫 행 자동.
                                               -- 사용자 편집 가능. NULL 허용.
  source      TEXT                              -- v22: 이전 markdown 리네임. nullable.
                                               -- 세션 전체 원본을 모드별 형식으로 누적:
                                               --  chat: 대화 로그 (user/assistant 섞임)
                                               --  markdown: 사용자가 친 마크다운 원문
                                               --  synapse: Q/A 로그
                                               --  insight: 본체 한 줄
...
sentences:                           -- 문장 하이퍼엣지의 실체 (post 에 반드시 속함)
  ...
  post_id     INTEGER NOT NULL REFERENCES posts(id) ON DELETE CASCADE
                                               -- v22: NOT NULL 강제. assistant 응답도
                                               -- 그 대화가 일어난 post 에 role='assistant'
                                               -- 로 귀속. 미아 sentence 없음.
  ...
  origin      TEXT                              -- v22 신설. NULL|'user'|'insight'.
...
nodes:                               -- v21: status 컬럼 폐기 (merge_nodes 물리 DELETE)
  id          INTEGER PRIMARY KEY
  name        TEXT NOT NULL          -- UNIQUE 아님. 동명이인/동명이의어 허용
  created_at  TEXT DEFAULT (datetime('now'))
  updated_at  TEXT DEFAULT (datetime('now'))

node_sentence_mentions:              -- 문장 하이퍼엣지의 멤버십 (v21 리네임 + origin, v17 'rule'→'system')
  ...
  -- v21: 이전 node_mentions 리네임. 이름이 "어디에 언급?" 모호했음을 해소.

node_category_mentions:              -- 노드 하이퍼엣지의 카테고리 멤버십 (v21 신설, v17 'rule'→'system')
  node_id     INTEGER REFERENCES nodes(id) ON DELETE CASCADE
  ...
  -- v21: 축 A(heading Kiwi 토큰화) + 축 B(19 대분류 LLM 워커) 통합. 저장 시
  -- _upsert_category_path 가 Kiwi nouns 를 자동 매핑 (origin='system'). CATEGORY
  -- 워커는 major_category (예 'BOD.medical') 를 categories.id 역매핑 후 origin='ai'.
...
- **posts = 세션 그릇 (v22 2차안)**: 한 개의 `posts` 행은 사용자 활동의 **세션 그릇**. 3 종 (`note`·`synapse`·`insight`) 공통. 그 안에 `sentences`가 누적되며, user·assistant 메시지가 `role` 로만 구분된다. post 는 사용자가 **명시적으로 나갈 때까지 열려있고** 목록에서 재진입·편집·이어서 쓰기 가능. 시간 기반 자동 경계 없음 (원칙 9 참조).
- **posts.source (v22)**: 세션 전체 원본 누적 보관. 모드별 형식이 다르지만 공통 목적은 ① UI 재편집 시 원형 그대로 로드, ② 지시어·날짜 치환 이전 원본 보존, ③ sentences 가 파싱·정규화된 가공본임을 감안한 **진짜로 친 원형**의 단일 출처. sentences 만으로 복원 불가능한 정보(heading·치환 전 표현)가 여기에만 있다.
- **sentences.post_id NOT NULL (v22)**: 모든 sentence 는 어떤 post 에 반드시 속한다. 이전엔 assistant 응답이 `post_id=NULL` 로 저장됐으나, v22 에서 "미아 sentence 없음" 원칙으로 전환 — 같은 대화의 사용자 메시지 post 에 귀속된다.
- **sentences.origin (v22)**: 기본 NULL 또는 `'user'`. 통찰 승격 sentence 는 `'insight'` 로 표식되며, 본문 편집 불가 (API/UI 레벨 강제). 원칙 15-2 참조. 삭제는 `/review` 승인 경로.
...
- **자동 저장 + origin**: `sentence_categories` / `node_category_mentions` / `aliases`는 전부 자동 저장되며, `origin` 컬럼(`user` / `ai` / `system` / `external`)으로 출처를 식별한다(`categories` 마스터는 origin 없음 — 삭제 경로 부재). v22 에서 `sentences.origin` 도 신설 (`'insight'` 승격 식별). 사용자는 UI·`/review`에서 origin 필터로 검수·수정·삭제할 수 있다. 파괴적 작업(노드 병합, 통찰 삭제 등)만 `/review` 승인을 거친다. (v17: 이전의 `rule` 값을 `system` 으로 리네이밍 — 출처는 데이터 생성 "주체"이며 규칙은 수단이기 때문)
...
- **posts.kind 사용자 명시(v19→v22)**: 그릇 종류를 사용자 액션으로 결정. v19 에서 `input_mode ∈ {chat, markdown}` 로 시작 → v22 1차안에서 `kind ∈ {chat, markdown, synapse, insight}` 4 값으로 확장 → **v22 2차안에서 입력 모드 분리 폐지로 `kind ∈ {note, synapse, insight}` 3 값**. 축적 그릇은 `note` 단일이며, 사용자는 입력 형식(평문 vs 마크다운)을 인지·선택하지 않는다. 입력 형식 분기는 LLM 의미 처리 단계가 흡수한다. 내용 기반 `has_heading()` 자동 분기 제거(v19) 정책은 그대로 유지. 상세는 `docs/DESIGN_INPUT_MODES_AND_RETRIEVAL.md`·`docs/DESIGN_PIPELINE.md`.
- **카테고리 재설계(v20→v21)**: v20 에서 단일 `node_categories` 를 두 축으로 분리 (heading 축 + 19 대분류 축). v21 에서 두 축을 `node_category_mentions`(`categories.id` FK) 단일 매핑으로 재통합 — 이중 저장 중복 해소. 저장 흐름은 heading 말단 카테고리만 `sentence_categories` 로 등록하고, 노드 ↔ 카테고리 멤버십은 `node_category_mentions` 에서 통합 처리된다. 상세는 `docs/DESIGN_CATEGORY.md`.
- **node_sentence_mentions**: "노드↔문장" 역참조 (v21 리네임). 시간·장소·부정 같은 특수 노드도 별도 취급 없이 여기만 참조.
- **unresolved_tokens**: 치환 실패한 지시어 기록. 저장 시점 일회성 이벤트라 런타임 재생성 불가 → 유일한 "승인 대기" 저장 예외. origin 불필요(사용자 답만 들어옴).
- **nodes·node_sentence_mentions·sentences 의 origin**: nodes·node_sentence_mentions 는 origin 없음 (원문 추출·자동 인덱스). sentences 는 v22 에서 origin 컬럼 신설 — 통찰 승격 식별용 (`NULL`/`'user'`/`'insight'`).
- `domain`, `weight`, `safety`, `filters`, `nodes.category`, `edges`(테이블 자체) 컬럼 없음.
