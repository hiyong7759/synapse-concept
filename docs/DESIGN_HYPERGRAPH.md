...
> - **리네임 `node_mentions` → `node_sentence_mentions`** — 이름이 모호했던 걸 명시화. `origin` 컬럼 추가 (`'rule'` 기본).
> - **신설 `node_category_mentions`** — 노드 ↔ 카테고리 멤버십. `_upsert_category_path` 가 heading 을 Kiwi 로 토큰화해 자동 매핑 (`origin='rule'`). AI 워커는 `origin='ai'`, 사용자 수정은 `'user'`.
...
node_category_mentions:              -- 노드 하이퍼엣지의 카테고리 멤버십 (v21 신설)
  ...
  -- _upsert_category_path 가 Kiwi nouns 를 자동 매핑 (origin='rule'). CATEGORY
