...
**최종 업데이트**: 2026-04-21 (v16 — Kiwi 형태소 분석 도입, 용언 lemma 정규화)
...
> v15 → v16 변경: **스키마 자체 변화 없음**. 노드 정체성 · 병합 규칙 ·
> 부정부사 감지 경로가 v16 에서 달라진 것 (Kiwi 형태소 분석 도입) 이고,
> 테이블 DDL·컬럼 집합은 v15 와 동일하다. 마이그레이션 불필요.
>
...
**2단 추출 (v16)** — LLM extract 와 Kiwi 형태소 분석을 병렬로 돌린 뒤 LLM 병합 단계에서 최종 노드를 결정한다. 자세한 흐름은 `docs/DESIGN_PIPELINE.md`, 프롬프트는 `docs/EXTRACT_SYSTEMPROMPT.md` · `docs/EXTRACT_MERGE_SYSTEMPROMPT.md`.
...
### 노드 정체성 — lemma 정규화 · 외래어 원형 유지

같은 개념은 반드시 같은 노드로 수렴해야 BFS 가 제대로 작동한다. 두 규칙으로 중복을 막는다.

- **한국어 용언은 Kiwi lemma** — `아파서 / 아프 / 아팠어` 모두 `아프` 로 정규화해 upsert. 활용형 파편화 방지.
- **외래어·영문·복합 고유명사는 원형 유지** — `React Native`, `MZ세대`, `L4-L5` 는 통째로 한 노드. Kiwi 가 글자 단위로 분리해도 LLM 병합 단계에서 원문의 원형으로 복원한다. 다국어 표기는 `aliases`(예: Wikidata `external`)로 연결.
