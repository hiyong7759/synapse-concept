...
**최종 업데이트**: 2026-04-23 (v19 계획 — PLAN-003 "마크다운 입력 체계 + 모드 분리" · `posts.input_mode` 컬럼 신설. 선행 v18: 상태 레이어 제거 / `sentences.status` 컬럼 폐기 / `extract-state` 폐기. v17: Kiwi-first + 메타 필터 + origin `rule→system`)
