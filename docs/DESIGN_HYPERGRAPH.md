...
posts:                               -- 세션 그릇 (v22 확장)
  ...
  kind        TEXT NOT NULL DEFAULT 'chat'
                  CHECK(kind IN ('chat','markdown','synapse','insight'))
                                               -- v22: input_mode 리네임 + 값 확장.
                                               -- chat/markdown: 지식 축적 (뉴런 재료)
...
- **posts = 세션 그릇 (v22)**: 한 개의 `posts` 행은 사용자 활동의 **세션 그릇**. 4 모드 공통. 그 안에 `sentences`가 누적되며, user·assistant 메시지가 `role` 로만 구분된다. post 는 사용자가 **명시적으로 나갈 때까지 열려있고** 목록에서 재진입·편집·이어서 쓰기 가능. 시간 기반 자동 경계 없음 (원칙 9 참조).
...
- **posts.kind 사용자 명시(v19→v22)**: 저장 모드를 사용자 명시로 기록. v19 에서 `input_mode ∈ {chat, markdown}` 로 시작, v22 에서 `kind ∈ {chat, markdown, synapse, insight}` 로 확장. 저장 파이프라인은 이 값을 보고 save-pronoun 호출 여부·메타 필터 대상 범위·parse_markdown 분기·노드 편입 여부를 결정한다. 내용 기반 `has_heading()` 자동 분기 제거 (v19). 상세는 `docs/DESIGN_INPUT_MODES_AND_RETRIEVAL.md`.
...
- `/chat` · `/compose` 목록에서 통찰 post 는 ✦ 아이콘과 앰버 보더로 구분
