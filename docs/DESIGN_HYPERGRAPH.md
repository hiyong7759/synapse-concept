...
**최종 업데이트**: 2026-04-24 (v21 — PLAN-007 "하이퍼엣지 매핑 단일화" · `node_mentions` → `node_sentence_mentions` 리네임 + origin 추가, `node_category_mentions` 신설 (축 A·B 통합), `node_categories` 폐기, `nodes.status` 폐기. 선행 v20: PLAN-004 카테고리 재설계 — `categories`·`sentence_categories` 신설. v19: `posts.input_mode` 신설. v18: 상태 레이어 제거. v17: Kiwi-first + 메타 필터 + origin `rule→system`)
