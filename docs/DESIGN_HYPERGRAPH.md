...
**최종 업데이트**: 2026-04-22 (v17 — Kiwi-first 저장 경로, origin `rule→system` 리네이밍, LLM extract/merge/structure-suggest 폐기)
...
> v16 → v17 변경: **DDL 변경 있음 — origin CHECK 제약 갱신**.
> `node_categories.origin` · `aliases.origin` CHECK 를
> `('user','ai','rule','external')` → `('user','ai','system','external')` 로 변경.
> 이유: `rule` 은 수단(규칙)이지 데이터 생성 주체가 아니며, 시스템 엔진이 주체 — `system` 이 정확.
> 기존 DB 는 PLAN-003 K6 합의 연장으로 **폐기 후 재생성**(2026-04-21 사용자 승인).
>
> v15 → v16: 스키마 동일. Kiwi 도입은 엔진 로직 수준 변경.
> v14 → v15 마이그레이션 정책은 아래 `engine/db.py` 의 자동 백업 로직 참고.
...
- **자동 저장 + origin**: `node_categories / aliases`는 전부 자동 저장되며, `origin` 컬럼(`user` / `ai` / `system` / `external`)으로 출처를 식별한다. 사용자는 UI·`/review`에서 origin 필터로 검수·수정·삭제할 수 있다. 파괴적 작업(노드 병합 등)만 `/review` 승인을 거친다. (v17: 이전의 `rule` 값을 `system` 으로 리네이밍 — 출처는 데이터 생성 "주체"이며 규칙은 수단이기 때문)
- **retention 폐기(v13)**: `sentences.retention` 컬럼 제거. 모든 sentence 동등하게 영구 보관.
- **sentences.status**: extract-state 가 새 발화로 인해 무효화된 문장을 `inactive`, 애매한 문장을 `pending` 으로 표시. 삭제하지 않음 — 영구 보관 원칙 유지, BFS 필터링 여부만 결정. 사용자는 `/review` 에서 pending 확정 / inactive 되돌리기 가능.
...
| `ai` | 백그라운드 워커(`engine/workers.py`)가 베이스 모델 + `CATEGORY_SYSTEMPROMPT.md` 로 분류 | `허리디스크` → `BOD.disease` |
| `system` | 결정론적 엔진 규칙 (v17 — 이전 `rule`) | `4월` → `TIM.month`, `안/못` → 부정부사 노드 |
...
doc_mode=True이면 조항 식별자(`제N조`, `제N항`, `N호`)를 노드로 두고, 같은 sentence 내 `node_mentions`로 본문 노드와 연결. 계층은 마크다운 heading 경로와 동일하게 `node_categories`(origin='system')로 표현한다.
...
모든 테이블 자동 저장. `node_categories / aliases`는 `origin` 컬럼으로 출처(user/ai/system/external)가 식별됨. `/review`는 (1) `unresolved_tokens`(애매한 지시어) 해소 + (2) AI·시스템·외부 생성물 목록 검수 + (3) 파괴적 작업(노드 병합) 승인 세 가지만 담당.
