...
- **세션리스**: `sessions` 테이블 없음. post 가 세션 그릇 역할을 수행한다. 하이퍼그래프가 영속 메모리, sentences 가 대화 기록. 매 입력마다 인출이 독립적으로 맥락 수집 (질의 확장 → 직접 lookup).
...
같은 개념은 반드시 같은 노드로 수렴해야 인출 lookup 이 제대로 작동한다. 규칙:
...
같은 sentence에 등장한 모든 노드가 하나의 **문장 하이퍼엣지**로 묶인다. 멤버십은 `node_sentence_mentions` 테이블이 담는다. 인출은 `node_sentence_mentions` JOIN으로 "같은 문장에 함께 언급된 노드"의 sentence 를 직접 수집한다.
...
질의 "스타벅스" → matchedNodeIds={스타벅스}
              → node_sentence_mentions JOIN sentences → {sid}
              → 그 sentence 의 본문 "스타벅스 안 좋아" 가 인출 컨텍스트로 들어감
              → 같이 언급된 {안, 좋아} 노드의 의미는 LLM 이 원문에서 해석
```

선 형태의 엣지 없이도 인출 연결성은 보장된다. 문장이 곧 "함께 활성화된 노드들의 묶음"이기 때문.
...
- `node_category_mentions`: 후보 `병원` 노드 → `BOD.medical` 시드 카테고리 id 확인 → `BOD.medical` 에 매핑된 모든 노드들의 `node_sentence_mentions` 를 직접 수집.
