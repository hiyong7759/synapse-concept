...
| M | 내용 | 주요 산출물 | 검증 |
|---|---|---|---|
| **F0** | **Flutter 전환 설계 정합** — 선행 PLAN M5 (입력 모드 분리 폐지) 위에 Flutter·2 계층 API·풀 오프라인 결정 반영. **DESIGN_ENGINE.md 대수술** (2 계층 API: SynapseFlow / LlmTasks / GraphOps + `allowedKinds`·`reservedKinds` 유연성 + 갑질 재사용 시나리오 + Kiwi WASM 통합). **DESIGN_MOBILE.md 보강** (v22 2차안 + 모바일 우선 + 데스크톱 통합). **나머지 7 문서** (PRINCIPLES·HYPERGRAPH·PIPELINE·INPUT_MODES·UI·APP·OVERVIEW) 의 풀 오프라인·Flutter 위젯·HTTP 폐기 표현 정합 | grep 으로 v22 1차안·"PC 데스크톱 기준"·"FastAPI" 등 잔재 0. 죽은 링크 0 |
...
| **F7** | `/note` 페이지 구현 | 사이드바 post 목록 (✦ insight 구분), 단일 입력 영역, 자동저장 (디바운스 1.5초), ⌘S 의미 처리 트리거, 정정 카드 인라인, 재진입·이어쓰기, post 삭제 | E2E (입력→자동저장→이탈→재진입→⌘S→정정 [적용]→source 갱신) |
| **F8** | `/synapse` 페이지 구현 | Q/A 스레드, 답변 카드 마크다운 렌더, `[⬆ 통찰로 승격]` 모달, 승격 후 `/note` 목록에 ✦ 표시 | 시냅스 세션 → 승격 → 허브 연결 검증 |
| **F9** | 2 라우트 E2E 통합 검증 | iOS·Android·macOS 빌드 각각에서 통합 시나리오 통과 | 모든 Acceptance 항목 통과 |
| **F10** | 문서·CLAUDE.md 갱신 | `docs/index.md` 에 Flutter 전환 반영, `CLAUDE.md` 의 명령 섹션 갱신, README 업데이트 | 링크·참조 무결 |
...
| M | 통과 조건 |
|---|---|
| **F0** | 설계 8 문서 grep 으로 v22 1차안·`PC 데스크톱 기준`·`FastAPI`·`/chat`·`/compose` 잔재 0. DESIGN_ENGINE 에 2 계층 API + `allowedKinds` + 갑질 재사용 시나리오 명기. DESIGN_MOBILE 에 v22 2차안 정합. 죽은 링크 0 |
...
| **F7** | E2E 시나리오 통과 (입력→자동저장→이탈→재진입→⌘S→정정 카드 [적용]→토큰 치환 + alias 등록) |
| **F8** | 시냅스 세션 → 승격 → ✦ 통찰 post 표시 + `node_sentence_mentions` 편입 검증 |
| **F9** | 3 플랫폼 (iOS·Android·macOS) 통합 시나리오 30분 내 통과 |
| **F10** | docs/index.md 링크 무결, CLAUDE.md 명령 섹션 갱신 완료 |
...
- `docs/DESIGN_MOBILE.md` — Flutter + llamadart 결정, 디바이스 벤치마크
