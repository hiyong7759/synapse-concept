...
| **F7a~c** | `/note` 페이지 구현 (그래프 패널 제외) | 사이드바 post 목록 (✦ insight 구분), 단일 입력 영역, 자동저장 (디바운스 1.5초), ⌘S 의미 처리 트리거, 정정 카드 인라인, 재진입·이어쓰기, post 삭제 | E2E (입력→자동저장→이탈→재진입→⌘S→정정 [적용]→source 갱신) |
| **F8** | `/hypergraph` 페이지 + **공유 `GraphView` 코어 컴포넌트** | 별도 라우트. **전체 누적 그래프** (모든 post 의 모든 노드/하이퍼엣지 통합 시각화), 허브 노드 강조, 노드 클릭 → 같은 바구니 멤버 + 원문 sentence, `origin='insight'` 강조, 검색·필터·BFS 깊이 슬라이더. **시각 정책: 옵션 ε (node-centric) — 노드가 시각의 주인공, 바구니는 클릭 시 우측 패널에 텍스트로**. 가장 어려운 케이스 (937+ 노드) 를 먼저 통과시켜 F7d / F9 가 같은 코어 컴포넌트를 `postId` / `nodeIds` 필터로 재사용. 라이브러리 = WebView + vis-network (DESIGN_UI line 410 명시). 시각 토큰 = Anthropic 디자인 시안 (`/tmp/synapse-design-bundle/synapse/project/tokens.css` 참고) | post 3 개 이상 입력 후 전체 그래프 렌더 + 노드 클릭 인터랙션 + 검색·필터·깊이 슬라이더 동작 |
| **F-bundle** | **모델 번들 + LLM 활성화 + `category` 태스크 신설** — Gemma 4 E2B-it Q4_K_M GGUF (외장 `/Volumes/macex/models/`, 환경변수 `SYNAPSE_MODEL_PATH`) 활성. `LlmTasks` 에 `categorize(nodeName, contextSentences) → seedCode` 추가 (DESIGN_APP §모델 line 38 명시 — `category` 가 6 LLM 태스크의 하나인데 코드 누락 상태). `noteProcess._categorizePostNodes` 가 호출되어 그 post 의 `origin='ai'` 매핑 없는 모든 노드를 분류 → `addCategoryMention(origin='ai')`. 모델 미도입 환경은 LLM null → categorize 호출 자체 skip (stub fallback). **기존 노드 backfill 및 비동기 처리는 F-bundle 7 으로 분리** (동기 ⌘S 1 회 처리는 938 노드 시 16~30 분 UI 막힘 → 사용 불가) | step 1~5 단위 테스트 통과. 실측 검증은 F-bundle 7 후 |
| **F-bundle 7** | **백그라운드 categorize 큐 + autosave 트리거 증분 + 부팅 backfill** — F-bundle step 4~5 의 동기 호출을 큐로 분리. (a) 단순 async 큐 + dedup. (b) autosave 시점 (1.5 초 idle) 에 그 post 의 미분류 노드만 큐 enqueue → 사용자가 글 쓰는 사이 분산. (c) `SynapseEngine.create()` 끝에 전체 미분류 노드 SELECT → 큐 enqueue (앱 켜면 천천히 backfill). (d) ⌘S 즉시 반환, FIFO. (e) 진척 표시 (autosave 칩 옆 작은 텍스트). KV 캐시 prefix 재사용은 llamadart 0.6.10 의 `reusePromptPrefix=true` 기본값으로 자동 — system 프롬프트 (~2500 토큰) 매 호출 무료. categorize 만 적용 (다른 LLM task 일반화는 별도 마일스톤) | macOS 점검: ⌘S 즉시 반환 (1 초 안). 글 입력 막힘 없음. 콘솔 진척 로그. 일정 시간 후 `/hypergraph` 19 색 분포. `node_category_mentions` ≥ 100 건. 단위 테스트 (큐 enqueue/dedup/순차 처리, note_pipeline 통합) |
| **F7d** | `/note` 그래프 패널 wrapper | `GraphView` 코어를 `postId` 필터 어댑터로 wrap. 데스크톱 우측 280px (⊟ 접기) / 모바일 `_showGraph` 토글. ⌘S 직후 invalidate | 본문 입력 + ⌘S → 그래프 패널에 노드 등장. 노트 갈음 → 그래프 갈음 |
| **F9** | `/synapse` 페이지 구현 | Q/A 스레드, 답변 카드 마크다운 렌더, `[⬆ 통찰로 승격]` 모달, 승격 후 `/note` 목록에 ✦ 표시, **해당 시냅스 세션 그래프 패널** (`GraphView` 코어를 `nodeIds` (retrieve 캐시) 필터로 wrap — 답변이 어떤 노드에서 끌어왔는지 추적) | 시냅스 세션 → 승격 → 허브 연결 검증 + 그래프 패널이 retrieve 노드 반영 |
...
**v22 2차안 정책: 단일 모델 + 어댑터 0 개 + 앱 번들 고정**. 모델 갈아끼우기·다운로드 인프라는 후속 PLAN (§2 제외 참고).

| 자원 | 사이즈 | 처리 |
|---|---|---|
| Gemma 4 E2B GGUF (Q4_K_M) | ~3.1GB | **iOS·Android·macOS·Windows 앱 번들** (다운로드 인프라 미도입) |
| 시스템프롬프트 5 종 (`CATEGORY`, `META_FILTER`, `RETRIEVE_EXPAND` v3, `RETRIEVE_FILTER`, `SYNAPSE_ANSWER`) | ~30KB | `assets/prompts/` 번들 |

→ 모바일 앱스토어 다운로드는 ~3.1GB. 사용자가 받기 부담 있으나 v22 2차안 단일 사용처(시냅스 본인) 단계에선 인프라 부담 없는 게 우선. 사용처 늘거나 모델 카탈로그 필요해지면 다운로드 인프라 후속 PLAN 에서 분리.

**어댑터 폐기**: 이전 v22 2차안 초안에는 `retrieve-expand` 어댑터 1 종 번들이 있었으나 2026-04-26 PoC 가 v3 시스템 프롬프트 + 베이스 모델로 동등 또는 우수함을 입증해 폐기 (산출: `deliverables/SYN/20260426/user/REPORT-20260426-SYN-adapter-removal-h1.md`). 어댑터 핫스왑 인프라 (`LlamadartInferenceBackend`) 는 보존 — 재사용 앱 (예: 갑질) 도메인 어댑터용.

**save-pronoun 폐기**: 이전 초안에는 `SAVE_PRONOUN_SYSTEMPROMPT` 도 있었으나 노트 단일 그릇 컨셉에서 ① 자연어 날짜 부사는 결정론적 `DateNormalizer` 가 ② 지시어는 `unresolved_tokens` 가 충분히 흡수해 폐기. 시스템 프롬프트는 `docs/archive/v22_retired/` 보존.

`archive/synapse_engine_v15/` 의 llamadart 통합 인프라 (모델 로딩·thinking 제거) 는 그대로 참고. LoRA 핫스왑 인프라는 재사용 앱용으로 보존.
...
| **F7a~c** | E2E 시나리오 통과 (입력→자동저장→이탈→재진입→⌘S→정정 카드 [적용]→토큰 치환 + alias 등록) |
| **F8** | post 3 개 이상 입력 후 `/hypergraph` 별도 라우트가 전체 누적 그래프 렌더 (옵션 ε node-centric, vis-network bipartite). 노드 = node_category_mentions 시드 7 색 (BOD/MND/FOD/WRK/REL/LIV/TIM), sentence 점 = 사용자 헤딩 root 별 색. 허브 노드 degree 비례 크기 + `origin='insight'` 노드 앰버. 노드 클릭 → 우측 패널에 같은 바구니 원문 sentence + 별칭 + 카테고리 + 검색·필터·BFS 깊이 슬라이더 동작. **시드 7 색 분포 검증은 F-bundle 후** (현재 LLM 분류 워커 부재로 색 매핑 0 건) |
| **F-bundle** | step 1~5 단위 테스트 통과. 실측 macOS 검증은 F-bundle 7 후 |
| **F-bundle 7** | macOS 점검 — `flutter run -d macos` → ⌘S 즉시 반환 + 글 입력 막힘 없음 + 콘솔 진척 로그. 일정 시간 (수 분) 후 `/hypergraph` 진입 시 노드가 19 색 분포로 보임. `node_category_mentions` ≥ 100 건. 큐 단위 테스트 통과 |
| **F7d** | F8 의 `GraphView` 코어를 `postId` 필터로 wrap → `/note` 우측 280px (⊟ 접기) / 모바일 `_showGraph` 토글. 본문 입력 + ⌘S → 그래프 패널이 새 노드 즉시 반영 |
| **F9** | 시냅스 세션 → 승격 → ✦ 통찰 post 표시 + `node_sentence_mentions` 편입 검증 + F8 의 `GraphView` 코어를 `nodeIds` (retrieve 캐시) 필터로 wrap → 그래프 패널이 retrieve 노드 반영 |
...
| GGUF 모델 ~3.1GB 앱 번들 사이즈 | 첫 실행 시 다운로드 옵션 검토 (네트워크 필요 1 회). 더 작은 양자화 (Q4_0 등) 검토 |
...
- F6 (앱 scaffold): 1 덩어리
- F7a~c (`/note` 그래프 제외) → F8 (`/hypergraph` + 공유 `GraphView` 코어, 시각 인프라까지) → **F-bundle (모델 활성 + `category` 태스크)** → **F-bundle 7 (백그라운드 큐 + autosave 트리거 — 사용 가능한 UX)** → F7d (`/note` 그래프 패널 wrapper) → F9 (`/synapse` + 그래프 패널 wrapper): 각 M 후 승인. F-bundle 의 동기 ⌘S 1 회 backfill 결정은 938 노드 × 1 초/호출 = 16 분 동기 대기로 사용 불가 — F-bundle 7 갈래 신설로 비동기 분산 처리. F7d / F9 는 같은 컴포넌트를 필터 어댑터로 재사용 (~~F7→F8 순서~~ 갈음, ~~F-bundle 을 F-late 로 미룸~~ 갈음)
