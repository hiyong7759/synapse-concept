...
| **F-bundle** | **모델 번들 + LLM 활성화 + `category` 태스크 신설** — Gemma 4 E2B-it Q4_K_M GGUF (외장 `/Volumes/macex/models/`, 환경변수 `SYNAPSE_MODEL_PATH`) 활성. `LlmTasks` 에 `categorize(nodeName, contextSentences) → seedCode` 추가 (DESIGN_APP §모델 line 38 명시 — `category` 가 6 LLM 태스크의 하나인데 코드 누락 상태). `noteProcess` 가 새 노드마다 호출 → `addCategoryMention(origin='ai')`. 기존 937 노드 회귀 분류 (CLI 또는 `/review` 액션). 모델 미도입 환경은 stub fallback (결정론적 규칙: 날짜 → TIM, 인칭 → PER) — 인터페이스 동일. 이 마일스톤 후에야 F8 의 시각 검증 (시드 7 색 분포) 이 의미를 가짐 | macOS 점검: `/hypergraph` 진입 → 노드가 시드 7 색 (BOD/MND/FOD/WRK/REL/LIV/TIM) 분포로 보임. `node_category_mentions` 0 건 → ≥ 100 건. 단위 테스트 (category 태스크 fake 1 케이스) |
