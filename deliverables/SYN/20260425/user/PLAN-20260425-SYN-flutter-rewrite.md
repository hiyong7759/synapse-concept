...
| **F-bundle** | **모델 번들 + LLM 활성화 + `category` 태스크 신설** — Gemma 4 E2B-it Q4_K_M GGUF (외장 `/Volumes/macex/models/`, 환경변수 `SYNAPSE_MODEL_PATH`) 활성. `LlmTasks` 에 `categorize(nodeName, contextSentences) → seedCode` 추가 (DESIGN_APP §모델 line 38 명시 — `category` 가 6 LLM 태스크의 하나인데 코드 누락 상태). `noteProcess._categorizePostNodes` 가 호출되어 그 post 의 `origin='ai'` 매핑 없는 모든 노드를 한 번에 분류 → `addCategoryMention(origin='ai')`. 별도 backfill 기능 없음 — **기존 노드는 사용자가 ⌘S 한 번 다시 눌러주는 것으로 채움** (해당 post 의 모든 미분류 노드 일괄 처리). 사용자 환경 (post 2 dominant, 937 노드) 에선 ⌘S 1 회로 사실상 전체 분류. 모델 미도입 환경은 LLM null → categorize 호출 자체 skip (stub fallback) | macOS 점검: 취업규칙 post 에서 ⌘S → 의미 처리 후 `/hypergraph` 진입 → 노드가 시드 7 색 (BOD/MND/FOD/WRK/REL/LIV/TIM) 분포로 보임. `node_category_mentions` 0 건 → ≥ 100 건. 단위 테스트 (category 태스크 5 + 파이프라인 통합 3 케이스) |
...
| **F-bundle** | macOS 점검 — `flutter run -d macos` → ⌘S 의미 처리 트리거 시 LLM categorize 호출 → `node_category_mentions` 행 ≥ 100 (회귀 분류 후), `/hypergraph` 진입 시 노드가 시드 7 색 분포로 보임. category 태스크 단위 테스트 통과 |
...
- F6 (앱 scaffold): 1 덩어리
- F7a~c (`/note` 그래프 제외) → F8 (`/hypergraph` + 공유 `GraphView` 코어, 시각 인프라까지) → **F-bundle (모델 활성 + `category` 태스크 + 회귀 분류)** → F7d (`/note` 그래프 패널 wrapper) → F9 (`/synapse` + 그래프 패널 wrapper): 각 M 후 승인. F8 을 먼저 통과시켜 가장 어려운 케이스 (937+ 노드) 에서 코어 검증 후 F-bundle 로 LLM 분류 채워야 시각이 살아남. F7d / F9 는 같은 컴포넌트를 필터 어댑터로 재사용 (~~F7→F8 순서~~ 갈음, ~~F-bundle 을 F-late 로 미룸~~ 갈음)
