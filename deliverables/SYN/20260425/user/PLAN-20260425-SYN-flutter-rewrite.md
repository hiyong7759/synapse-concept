...
| **F8** | post 3 개 이상 입력 후 `/hypergraph` 별도 라우트가 전체 누적 그래프 렌더 (옵션 ε node-centric, vis-network) + 허브 노드 강조 (degree 비례 크기) + `origin='insight'` 노드 앰버 + 카테고리 색상 (BOD/WRK/FOD/REL/MED/PLC/TIM) + 노드 클릭 → 우측 패널에 같은 바구니 원문 sentence + 별칭 + 카테고리 + 검색·필터·BFS 깊이 슬라이더 동작 |
...
- F6 (앱 scaffold): 1 덩어리
- F7a~c (`/note` 그래프 제외) → F8 (`/hypergraph` + 공유 `GraphView` 코어) → F7d (`/note` 그래프 패널 wrapper) → F9 (`/synapse` + 그래프 패널 wrapper): 각 M 후 승인. F8 을 먼저 통과시켜 가장 어려운 케이스 (937+ 노드) 에서 코어 검증 후 F7d / F9 가 같은 컴포넌트를 필터 어댑터로 재사용 (~~F7→F8 순서~~ 갈음)
