...
단, `category`는 예외 — BFS 검색 범위 필터링을 위해 허용. `node_categories` 테이블로 다대다 관리 (노드당 복수 카테고리 가능). 두 종류: 사용자 지정 경로(마크다운 heading, 예: `더나은.개발팀`) 또는 LLM 기본 분류(`대분류.소분류`, 예: `BOD.disease`). 마크다운 입력 시 heading 경로가 우선, 평문은 DB 매칭 후 LLM 폴백. 카테고리는 누적 — 같은 노드를 다른 맥락에서 만나면 카테고리가 추가됨.

## DB 스키마 (v10 — 세션리스 + 다대다 카테고리)
...
- node_categories: 노드당 복수 카테고리 (다대다). 사용자 지정 경로 (예: `더나은.개발팀`) 또는 LLM 기본 분류 (`대분류.소분류`, 예: `BOD.disease`). INSERT OR IGNORE로 중복 방지.
...
- domain, source, weight, safety, edges.type, filters, filter_rules, nodes.category 없음 (v6~v10에서 제거/분리).
...
engine/          ← Python 패키지. 그래프 저장/인출 파이프라인.
  db.py          ← v10 세션리스 스키마 + node_categories 다대다 + 마이그레이션 + 연결 관리
...
[마크다운 모드] heading(#) 있음:
  각 (경로, 항목)마다:
    ...
    → category를 heading 경로로 node_categories에 추가 (기존 카테고리 보존)
...
[평문 모드] heading 없음:
  ...
  [2단계: 저장]
    ...
    → [규칙 기반] category DB 매칭 (node_categories에서 사용자 지정 경로 우선, 없으면 LLM 폴백) → 누적 추가
    → [규칙 기반] 부정부사 후처리, 오타 교정
    → DB 저장 + 별칭 제안
...
질문
  ...
  → 카테고리 보완: node_categories에서 시작 노드 소분류 → 인접 소분류 미방문 노드 추가 조회 → 재필터
  → [synapse/chat] 자연어 답변
```
