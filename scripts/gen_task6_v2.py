...
전처리 전제: 이 파이프라인에 도달하기 전에 이전 단계(Task 2)에서 모든 부사·지시어가 구체적인 값으로 치환되었다.
- 시간 부사(오늘/어제/지난달/이번 주 등) → 실제 날짜(2026-04-10, 2026-03, 2026-W14 등)
- 장소 지시어(거기서/그곳에서/여기서 등) → 구체 장소명(강남역에서, 판교에서 등)
- 인물 지시어(걔/그분/그사람/쟤 등) → 구체 인물명(박지수, 팀장님 등)
- 치환 불가능한 모호한 표현은 사용자에게 재확인 후 처리되므로 이 단계에 도달하지 않는다.
...
- retention: 잘 변하지 않는 사실/상태/이력(직업·거주지·관계·질병 이력·기술 등) → "memory". 순간적 활동/감정/일상(기분 등) → "daily".
...
입력:
나 2025-08-15에 판교로 이사했어
알려진 사실:
- 나 성수동에서 살고 있어
출력: {"retention":"memory","nodes":[{"name":"나","category":"PER.individual"},{"name":"2025-08-15","category":"LIV.moving"},{"name":"판교","category":"LIV.housing"}],"edges":[{"source":"나","label":"에","target":"2025-08-15"},{"source":"나","label":"으로","target":"판교"}],"deactivate":[{"source":"나","target":"성수동"}]}

입력:
나 1977년생이야
알려진 사실: 없음
출력: {"retention":"memory","nodes":[{"name":"나","category":"PER.individual"},{"name":"1977년생","category":"PER.individual"}],"edges":[{"source":"나","label":null,"target":"1977년생"}],"deactivate":[]}
...
- 엄마가 고혈압이야
- 엄마가 아침마다 혈압약 먹어
출력: {"retention":"daily","nodes":[{"name":"엄마","category":"PER.family"},{"name":"혈압약","category":"BOD.medical"}],"edges":[{"source":"엄마","label":null,"target":"혈압약"}],"deactivate":[]}

입력:
내 운동 스케줄 기억해둬 월수금 헬스장
알려진 사실: 없음
출력: {"retention":"memory","nodes":[{"name":"나","category":"PER.individual"},{"name":"운동 스케줄","category":"HOB.sport"},{"name":"월수금","category":"HOB.sport"},{"name":"헬스장","category":"HOB.sport"}],"edges":[{"source":"나","label":"의","target":"운동 스케줄"},{"source":"운동 스케줄","label":null,"target":"월수금"},{"source":"운동 스케줄","label":null,"target":"헬스장"}],"deactivate":[]}\
"""
...
치환 전제: 이 파이프라인에 도달하기 전에 모든 시간 부사·장소 지시어·인물 지시어가 구체값으로 치환됨. sentence 생성 시 오늘/어제/거기서/걔 등 모호한 부사·지시어 사용 금지. 구체화 불가능한 표현은 아예 제거.
...
- 시간 부사(오늘/어제/요즘/최근/지난달/이번 주/작년/올해/방금/아까/지금 등) 사용 금지 → 실제 날짜/시각으로 (2026-04-10, 2026-04-10 14:30, 2025-08-15 등)
- 장소 지시어(거기서/그곳에서/여기서 등) 사용 금지 → 구체 장소명으로
- 인물 지시어(걔/그분/그사람/쟤 등) 사용 금지 → 구체 인물명으로
- 나이 표현은 출생년도로 치환됨: "49살" → "1977년생", "7살" → "2019년생"
- 구체화할 수 없는 모호한 부사는 문장에서 아예 제거."""


def build_gen_prompt_questions(group: dict, n: int) -> str:
    """의문문 보충 생성 전용."""
    topics_str = "\n".join(f"  - {t}" for t in group["topics"])
    deactivate_n = max(2, int(n * 0.15))
    daily_n = max(2, int(n * 0.20))
    normal_n = n - deactivate_n - daily_n

    return f"""\
Synapse 지식 그래프 파인튜닝 데이터를 생성하라. 의문문(질문) 전용.
...
[
  {{
    "sentence": "한국어 의문문",
    "context_sentences": ["과거에 저장된 관련 문장"],
    "answer": {{
...
모든 sentence는 의문문(~지?, ~야?, ~어?, ~나?, ~까?, ~래?, ~던데? 등)이어야 한다.

생성 비율:
- 노드/엣지 추출 가능한 의문문 (context 있음): {normal_n}개
  예: "엄마 혈압약 뭐 먹고 있었지?" → 엄마, 혈압약 노드 추출
- 상태변경 확인 의문문 (deactivate 있음): {deactivate_n}개
  예: "민수 아직도 쿠팡 다녀?" + context["민수 쿠팡에서 일해"] → 추출만, deactivate 없음 (변한게 아님)
- 추출 없는 일상 질문 (daily, nodes=[]): {daily_n}개
  예: "잘 잤어?", "밥 먹었어?" → 빈 결과

핵심: 의문문이라도 언급된 노드/엣지는 추출한다. 질문 자체는 daily지만, 사실을 포함하면 memory.
예: "나 1977년생인데 연금 언제부터 받을 수 있어?" → memory (1977년생 사실 포함)
...
def build_gen_prompt_commands(group: dict, n: int) -> str:
    """명령문 보충 생성 전용."""
    topics_str = "\n".join(f"  - {t}" for t in group["topics"])

    return f"""\
Synapse 지식 그래프 파인튜닝 데이터를 생성하라. 명령문/요청문 전용.
대상 카테고리: {group['categories']}
주제 가이드:
{topics_str}
...
모든 sentence는 명령문/요청문(~해줘, ~기억해, ~기록해, ~해, ~알려줘, ~찾아줘 등)이어야 한다.

유형:
- 기억 요청: "내 운동 스케줄 기억해둬 월수금 헬스장" → 노드 추출, memory
- 정보 요청: "엄마 병원 예약 일정 알려줘" → 노드 추출, daily
- 상태 기록: "2026-04-10부터 금연 시작이야 기록해" → 노드 추출, memory
- 단순 명령: "알겠어 고마워" → 빈 결과, daily

명령문 비율: 기억/기록 요청 60%, 정보/검색 요청 30%, 빈 결과 10%
...
def build_gen_prompt_birthyear(n: int) -> str:
    """나이→출생년도 패턴 보충 생성 전용."""
    return f"""\
Synapse 지식 그래프 파인튜닝 데이터를 생성하라. 나이/출생년도 패턴 전용.

아래 형식으로 정확히 {n}개 예시를 JSON 배열로 출력하라. 다른 텍스트 금지.

[
  {{
    "sentence": "한국어 문장 (출생년도 포함)",
    "context_sentences": [],
    "answer": {{
      "retention": "memory",
      "nodes": [{{"name":"노드명","category":"대분류.소분류"}}],
...
전처리 전제: 나이 표현은 이전 단계에서 출생년도로 치환됨.
- "나 49살이야" → "나 1977년생이야"
- "아들이 7살이야" → "아들이 2019년생이야"
- "할머니가 80세야" → "할머니가 1946년생이야"

따라서 sentence에는 "N살/N세" 대신 "YYYY년생" 형태가 와야 한다.

다양한 패턴:
- 본인 나이: "나 1977년생이야" (단순)
- 가족 나이: "아들이 2019년생이야" (가족 관계)
- 나이 비교: "나 1990년생이고 남편은 1988년생이야" (복합)
- 나이 + 상태변경: "엄마가 1960년생인데 2026-03에 퇴직했어" + context → deactivate 포함
- 나이만 언급 (context 없음): 70%
- 나이 + 상태변경 (context 있음): 30%

카테고리: 나이/출생년도 자체는 PER.individual. 관련 맥락에 따라 다른 카테고리도 사용.
...
부정부사+단순 상태변경 예시 (안/못은 반드시 독립 노드):
...
def generate_supplement(mode: str, target: int, dry_run: bool = False) -> int:
    """보충 생성: questions, commands, birthyear."""
    out_file = DATA_DIR / f"task6_v2_supp_{mode}.jsonl"
    ...
    if generated >= target:
        print(f"[SUPP-{mode.upper()}] 이미 완료 ({generated}건)")
        return generated

    remaining = target - generated
    batches = (remaining + BATCH_SIZE - 1) // BATCH_SIZE
    print(f"[SUPP-{mode.upper()}] 시작: {generated}/{target}건 — {batches}회 호출 예정")

    if dry_run:
        print(f"  [DRY RUN] {batches}회 × ~{BATCH_SIZE}건")
        return generated

    # 그룹 순환 (의문문/명령문은 그룹별로 분산)
    group_keys = ["a", "b", "c", "d"]
    group_idx = 0
    ...
    with open(out_file, "a", encoding="utf-8") as fp:
        ...
        while generated < target:
            ...
            group = GROUPS[group_keys[group_idx % len(group_keys)]]
            group_idx += 1

            if mode == "questions":
                prompt = build_gen_prompt_questions(group, batch_n)
            elif mode == "commands":
                prompt = build_gen_prompt_commands(group, batch_n)
            elif mode == "birthyear":
                prompt = build_gen_prompt_birthyear(batch_n)
            else:
                raise ValueError(f"Unknown mode: {mode}")

            print(f"  [SUPP-{mode.upper()}] {generated}/{target}건 — {batch_n}건 요청 중...")
            raw = call_claude(prompt)
            examples = parse_examples(raw)

            if not examples:
                retry += 1
                print(f"  [SUPP-{mode.upper()}] 파싱 실패 — 재시도 {retry}/{MAX_RETRIES}")
                if retry >= MAX_RETRIES:
                    print(f"  [SUPP-{mode.upper()}] 최대 재시도 초과, 중단")
                    break
                continue
            retry = 0
            ...
            print(f"  [SUPP-{mode.upper()}] +{new_count}건 저장 (누계 {generated}/{target}건)")

    print(f"[SUPP-{mode.upper()}] 완료: {generated}건 → {out_file.name}")
    return generated
...
def main():
    ...
    parser.add_argument("--supplement", choices=["questions", "commands", "birthyear", "all"],
                        help="보충 생성 모드")
    parser.add_argument("--supp-target", type=int, help="보충 생성 목표 건수")
    ...
    if args.supplement:
        supp_targets = {
            "questions": args.supp_target or 150,
            "commands": args.supp_target or 80,
            "birthyear": args.supp_target or 20,
        }
        modes = list(supp_targets.keys()) if args.supplement == "all" else [args.supplement]

        total = 0
        for mode in modes:
            try:
                total += generate_supplement(mode, supp_targets[mode], args.dry_run)
            except Exception as e:
                print(f"[SUPP-{mode.upper()}] 오류: {e}")

        print(f"\n보충 생성 완료: {total}건")
        for mode in modes:
            f = DATA_DIR / f"task6_v2_supp_{mode}.jsonl"
            if f.exists():
