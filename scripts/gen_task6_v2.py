...
변경사항:
  - "나" 노드 제거 (개인 모드: 1인칭 → 노드 미생성)
  - deactivate 필드 추가 (상태변경 감지 통합)
  - input에 "현재 그래프:" 맥락 포함 (deactivate 학습용)
...
- "현재 그래프:"가 제공된 경우: 입력과 상충되는 기존 엣지를 deactivate에 포함. 없으면 [].
...
현재 그래프:
- 허리 → 아프
출력: {"retention":"memory","nodes":[],"edges":[],"deactivate":[{"source":"허리","target":"아프"}]}

입력:
박지수가 개발팀장으로 승진했어
현재 그래프:
- 박지수 → 팀원
출력: {"retention":"memory","nodes":[{"name":"박지수","category":"PER.colleague"},{"name":"개발팀장","category":"WRK.role"}],"edges":[{"source":"박지수","label":"으로","target":"개발팀장"}],"deactivate":[{"source":"박지수","target":"팀원"}]}

입력:
오늘 삼겹살 먹었어
현재 그래프: 없음
출력: {"retention":"daily","nodes":[{"name":"삼겹살","category":"FOD.meat"}],"edges":[],"deactivate":[]}
...
[
  {{
    ...
    "current_graph": [{{"source":"노드","target":"노드"}}],
    "answer": {{
      "retention": "memory",
...
생성 비율:
- 신규 조항 추출 (current_graph=[]): {normal_n}개
- 조항 개정 (deactivate 있음 — 기존 조항이 새 내용으로 대체): {deactivate_n}개
...
def build_gen_prompt(group: dict, n: int, done_sentences: set[str] | None = None) -> str:
...
생성 비율:
- 일반 추출 (current_graph=[], retention="memory"): {normal_n}개
- 상태변경 포함 (deactivate 있음, current_graph에 기존 엣지 포함): {deactivate_n}개
...
- 엣지의 source와 target은 반드시 nodes 배열에 있는 노드명과 정확히 일치해야 한다.
- 상태변경 문장: current_graph에 비활성화할 엣지를 포함하고, deactivate에 명시.
...
def make_user_content(sentence: str, current_graph: list[dict]) -> str:
    if not current_graph:
        return f"{sentence}\n현재 그래프: 없음"
    lines = "\n".join(f"- {e['source']} → {e['target']}" for e in current_graph)
    return f"{sentence}\n현재 그래프:\n{lines}"
...
def to_jsonl_record(example: dict) -> str | None:
    try:
        ...
        current_graph = example.get("current_graph", [])
        ...
        user_content = make_user_content(sentence, current_graph)
    ...
    with open(out_file, "a", encoding="utf-8") as fp:
        ...
        while generated < per_group:
            ...
            if group.get("doc_mode"):
                ...
                prompt = build_gen_prompt(group, batch_n, done_sentences)
