...
치환 전제 (sentence 생성 시 반드시 준수):
- 이 파이프라인에 도달하기 전에 이전 단계(Task 2)에서 모든 부사·지시어가 구체값으로 치환됨.
- 시간 부사(오늘/어제/지난달/이번 주/작년 등) 사용 금지 → 실제 날짜/연도/주차로 (2026-04-10, 2026-03, 2025년 등)
- 장소 지시어(거기서/그곳에서/여기서 등) 사용 금지 → 구체 장소명으로 (강남역에서, 판교에서 등)
- 인물 지시어(걔/그분/그사람/쟤 등) 사용 금지 → 구체 인물명으로 (박지수, 팀장님 등)
- 구체화할 수 없는 모호한 부사는 문장에서 아예 제거. 치환하거나 빼거나 둘 중 하나.
