...
브라우저
    ...
    │      맥락을 system message로 구성
    │
    ├─ [3] Groq API 직접 호출 (프록시 경유)
    │      POST /api/llm/v1/chat/completions
    │      { model: "openai/gpt-oss-120b", messages: [...], stream: true }
    │
    ├─ [4] SSE 스트리밍 → 실시간 렌더링 (마크다운, 코드 블록)
    │
    └─ [5] 대화 로그를 브라우저 로컬 sessions.db에 저장
```

그래프 탐색, 맥락 조립, 대화 로그 저장은 전부 브라우저에서.
프록시는 Groq API 키를 숨기기 위한 것일 뿐, 맥락 내용을 모름.
...
OpenAI 호환 API (Groq):
  ...
  SSE 응답:
  data: {"choices":[{"delta":{"content":"패"}}]}
  data: {"choices":[{"delta":{"content":"키지"}}]}
  ...
  data: [DONE]
```
...
```
대화:     openai/gpt-oss-120b
비전:     meta-llama/llama-4-scout-17b-16e-instruct
NLI L2:  llama-3.3-70b-versatile (기존 유지)
```

### 대화 이력 관리

```
Groq는 세션 유지 없음. 매 호출마다 messages 배열 전체 전송.
  → 브라우저에서 messages 배열 관리
  → Synapse 맥락은 system message로 주입
  → 대화 로그는 sessions.db (브라우저 로컬)에 저장

messages 구조:
  [
    { role: "system", content: "[사용자 맥락 정보]\n..." },   ← Synapse
    { role: "user", content: "상태관리 뭐 쓸까?" },
    { role: "assistant", content: "Zustand 경험이..." },
    { role: "user", content: "그거 예제 보여줘" },            ← 이어지는 대화
  ]
```
...
NLI LLM (L2):       llama-3.3-70b-versatile (Groq, 기존 유지)
AI Chat:             openai/gpt-oss-120b (Groq)
비전:                meta-llama/llama-4-scout-17b-16e-instruct (Groq)
```
...
Groq API 키를 브라우저에 노출하지 않기 위해 프록시 필요.
기존 NLI에서 사용하던 `/api/llm/v1` 프록시를 그대로 활용.

```
Vite 프록시 (vite.config.ts):
  /api/llm/v1/* → Groq API (https://api.groq.com/openai/v1/*)
  서버에서 GROQ_API_KEY 헤더 주입
```

NLI L1 패턴 매칭은 브라우저에서 직접 처리 (프록시 불필요).
NLI L2, AI Chat 모두 같은 Groq 프록시 사용 (모델만 다름).
